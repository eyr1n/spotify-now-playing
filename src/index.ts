...
app.get("/", async (c) => {
  if (c.env.REDIRECT_URL) {
    ...
    const res: { access_token: string; refresh_token: string } = await fetch(
      "https://accounts.spotify.com/api/token",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
          Authorization: `Basic ${btoa(c.env.CLIENT_ID + ":" + c.env.CLIENT_SECRET)}`,
        },
    ...
    await c.env.KV.put("access-token", res.access_token);
    await c.env.KV.put("refresh-token", res.refresh_token);
  ...
  const accessToken = await c.env.KV.get("access-token");
  if (!accessToken) {
    throw new Error("access-token is null");
  }

  const res: SpotifyAPIResponse = await fetch(
  ...
  const nowPlaying: SpotifyNowPlaying = {
    album: {
      ...
      url: res?.item?.album?.external_urls.spotify,
    },
    artists:
      res.item?.artists?.map((artist) => ({
        name: artist.name,
        url: artist.external_urls?.spotify,
      })) ?? [],
    duration_ms: res.item?.duration_ms,
    images:
      res.item?.album?.images.map((image) => ({
        url: image.url,
        height: image.height,
        width: image.width,
      })) ?? [],
